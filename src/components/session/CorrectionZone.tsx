...
export function CorrectionZone({ sessionId, mode, onQuestionLink, pdfLinks = [], onNavigateToLink, onUserModeChange }: CorrectionZoneProps) {
  ...
  const allUserQuestions = () => {
    ...
    const clinical = (data.clinicalCases||[]).flatMap((c:any)=> (c.questions||[]).map((q:any)=>({ kind:'clinical', parentId:c.id, q })));
  ...
  return (
    <div className="space-y-4 w-full max-w-4xl mx-auto h-full">
      {/* Actions / header card */}
          ...
          <div className="flex gap-2 flex-wrap items-center justify-center sm:justify-start">
            {isEditor && (
              <>
                {canEdit && (
                  <>
                    <Button size="sm" variant="outline" onClick={addMedicalQCMQuestion} className="gap-2 bg-blue-50 hover:bg-blue-100 border-blue-200 text-blue-800 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:border-blue-800 dark:text-blue-200 min-w-[140px] font-medium">
                      <ClipboardList className="h-4 w-4" /> 
                      QCM Médical
                    </Button>
                    <Button size="sm" variant="outline" onClick={addMedicalQROCQuestion} className="gap-2 bg-green-50 hover:bg-green-100 border-green-200 text-green-800 dark:bg-green-900/30 dark:hover:bg-green-900/50 dark:border-green-800 dark:text-green-200 min-w-[140px] font-medium">
                      <PenLine className="h-4 w-4" /> 
                      QROC Médical
                    </Button>
                    <Button size="sm" variant="outline" onClick={addClinicalCase} className="gap-2 bg-purple-50 hover:bg-purple-100 border-purple-200 text-purple-800 dark:bg-purple-900/30 dark:hover:bg-purple-900/50 dark:border-purple-800 dark:text-purple-200 min-w-[140px] font-medium">
                ...
                {canEdit && (
                  <div className="flex items-center gap-2">
                    <Button size="sm" onClick={saveCorrectionData} disabled={saving} className="gap-2 bg-primary hover:bg-primary/90 text-primary-foreground min-w-[120px] disabled:opacity-70">
                ...
                >
                  {editorTestMode ? 'Quitter mode utilisateur' : 'Mode utilisateur'}
                </Button>
              </>
            )}
            {/* In user view we no longer show answer / validate buttons here to force full completion first */}
          </div>

          {/* Scoring UI removed */}

          {/* Global Score Display */}
          {isValidated && validationResults && (!isEditor || editorTestMode) && (
            <div id="score-display" ref={scoreDisplayRef} className="mt-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 border border-green-200 dark:border-green-700 rounded-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-6 w-6 text-green-600 dark:text-green-400" />
                  <div>
                    <h3 className="font-bold text-lg text-gray-800 dark:text-gray-200">
                      Score Total: {validationResults.totalScore.correct} / {validationResults.totalScore.total}
                    </h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {validationResults.totalScore.percentage >= 80 ? "Excellent travail !" :
                       validationResults.totalScore.percentage >= 60 ? "Bien joué !" :
                ...
                <div className="text-right">
                  <div className={cn(
                    "text-3xl font-bold",
                    validationResults.totalScore.percentage >= 80 ? "text-green-600 dark:text-green-400" :
                    validationResults.totalScore.percentage >= 60 ? "text-orange-600 dark:text-orange-400" :
                    "text-red-600 dark:text-red-400"
                  )}>
                    {validationResults.totalScore.percentage}%
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {validationResults.totalScore.percentage >= 80 ? "Réussite" :
                     validationResults.totalScore.percentage >= 60 ? "Passable" :
                     "À améliorer"}
                  </div>
                </div>
              </div>
              <div className="mt-3 bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                <div 
                  className={cn(
                    ...
                    validationResults.totalScore.percentage >= 80 ? "bg-green-500" :
                    validationResults.totalScore.percentage >= 60 ? "bg-orange-500" :
                    "bg-red-500"
                  )}
                  style={{ width: `${validationResults.totalScore.percentage}%` }}
                />
              </div>
            </div>
          )}
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  {canEdit ? (
                    <Input
                      ...
                      className="h-8 flex-1 max-w-md bg-white/80 dark:bg-gray-800/50 border-gray-300 dark:border-gray-600 focus:ring-blue-500"
                    />
                  ) : (
                    <div className="flex-1 max-w-md text-sm sm:text-base font-medium text-blue-100/90 dark:text-blue-200/90 bg-blue-900/20 border border-transparent rounded-md px-3 py-1.5 leading-snug select-text">
                      {(() => {
                    ...
                    <>
                      ...
                      {!existingLink && isEditor && !editorTestMode && (
                        <Button
                          ...
                          className="gap-1 text-xs border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 flex-shrink-0"
                        >
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  {canEdit ? (
                    <Input
                      ...
                      className="h-8 flex-1 max-w-md bg-white/80 dark:bg-gray-800/50 border-purple-300 dark:border-purple-600 focus:ring-purple-500 font-semibold"
                    />
                  ) : (
                    <div className="flex-1 max-w-md text-sm sm:text-base font-semibold text-purple-100/90 dark:text-purple-200/90 bg-purple-900/20 border border-transparent rounded-md px-3 py-1.5 leading-snug select-text">
          ...
          <CardContent className="space-y-4 px-4 sm:px-6">
            ...
            <div className="space-y-2">
              ...
              {canEdit ? (
                ...
                <div className="p-3 bg-purple-50/50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-md text-sm sm:text-base text-purple-800 dark:text-purple-200 whitespace-pre-wrap">
  ...
  const answeredClinical = (userAnswers.clinicalCaseAnswers||[]).reduce((acc,c)=>acc + (c.questionAnswers||[]).length,0);
      ...
      {isUserView && isValidated && reviewIndex !== null && pendingSelfEvalIds.length > 0 && (
        <div className="fixed top-4 right-4 z-50 bg-blue-600 text-white px-4 py-3 rounded-lg shadow-lg border border-blue-500">
          ...
          <div className="text-xs space-y-1">
            ...
            <div className="border-t border-blue-400 pt-1 mt-2">Entrée = Suivant</div>
          </div>
        </div>
      )}
    </div>
